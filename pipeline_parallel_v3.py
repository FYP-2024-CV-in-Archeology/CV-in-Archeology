...
if __name__ == "__main__":
    ...
    # create processes
    processes = [
        Process(target=imread, args=(pathes_queue, queues[0])),
        Process(target=detect24, args=(queues[0], queues[1])),
        Process(target=whitebalance, args=(queues[1], queues[2], num)),
        Process(target=color_correct, args=(queues[2], queues[3])),
        Process(target=color_correct, args=(queues[2], queues[3])),
        Process(target=imwrite, args=(queues[3], sizes, 2)),
        Process(target=imread, args=(pathes_queue, queues[0])),
        Process(target=detect24, args=(queues[0], queues[1])),
        Process(target=whitebalance, args=(queues[1], queues[2], num)),
        Process(target=color_correct, args=(queues[2], queues[3])),
