import os
import sys
import time
import cProfile

sys.path.append('../')

import numpy as np
import tkinter as tk
from pathlib import Path
import cv2 as cv
import rawpy

from multiprocessing import Process
from multiprocessing import Queue
from multiprocessing import Pipe

import utils
from cropping import detectSherd, crop
from scaling import scaling, calc_scaling_ratio
from color_correction import color_correction, percentile_whitebalance

def imread(input_path, queue_out):
    print(f"imread Started!")
    
    for dirpath, dirnames, filenames in os.walk(input_path):
        dirnames.sort()
        for file in filenames:
            path = Path(dirpath) / file
            dir, filename = path.parent.name, path.stem
            
            if '.DS_Store' in filename or 'pxrf' in filename:
                continue
            filename = filename[0]

            if dir > '478130_4419430_8_20':
                if 'cr' in path.suffix.lower() and (filename == '2' or filename == '1'):
                    queue_out.send([path, utils.imread(path)])
                    print(f"imread finished {path}")
                    # sleep(999999.9999)

    print(f"imread Done!")
    queue_out.send(None)

def detect24(queue_in, queue_out):
    print(f"detect24 Started!")

    while True:
        data = queue_in.recv()
        if data is None:
            queue_out.send(data)
            break
        
        path, raw_img = data
        detector = cv.mcc.CCheckerDetector_create()
        is24Checker = utils.detect24Checker(cv.cvtColor(raw_img.copy(), cv.COLOR_RGB2BGR), detector)  # must be bgr
        scaling_ratio = calc_scaling_ratio(raw_img.copy(), is24Checker, 900)

        queue_out.send([path, raw_img, is24Checker, scaling_ratio]) 
        print(f"detect24 finished {path}")

    print(f"detect24 Done!")

def whitebalance(queue_in, queue_out):
    print(f"Whitebalance Started!")

    while True:
        data = queue_in.recv()
        if data is None:
            queue_out.send(data)
            break

        path, raw_img, is24, scalin_ratio = data

        white_balance_img = raw_img if is24 else percentile_whitebalance(raw_img, 97.5)
        # if flag:
        #     whiteBalancedImg = cv.add(whiteBalancedImg, (10,10,10,0))
        sherd_cnt = detectSherd(white_balance_img, is24)
        queue_out.send([path, white_balance_img, is24, scalin_ratio, sherd_cnt])
        print(f"Whitebalance finished {path}")

    print(f"Whitebalance Done!")

def color_correct(queue_in, queue_out):
    print(f"Color Correction Started!")

    while True:
        data = queue_in.get()
        if data is None:
            queue_out.put(data)
            break

        path, white_balance_img, is24, scalin_ratio, sherd_cnt = data

        detector = cv.mcc.CCheckerDetector_create()
        color_correct_img = color_correction(white_balance_img, detector, is24)
        
        queue_out.send([path, color_correct_img, is24, scalin_ratio, sherd_cnt])
        print(f"Color Corrected {path}")

    print(f"Color Correction Done!")

def imwrite(queue_in):
    print(f"Write Started!")

    while True:
        data = queue_in.recv()
        if data is None:
            break

        path, color_correct_img, is24, scalin_ratio, sherd_cnt = data
        cropped_img = scaling(crop(color_correct_img, sherd_cnt, scalin_ratio), scalin_ratio)
        cv.imwrite(f"outputs/{path.stem}.jpg", cropped_img)
        print(f"Write finished {path}")
        
    print(f"Write Done!")
    

if __name__ == "__main__":
    start_time = time.time()
    # create pipes
    conn1, conn2 = Pipe()
    conn3, conn4 = Pipe()
    conn5, conn6 = Pipe()

    # create processes
    processes = [
        Process(target=imread, args=('test', conn2)),
        Process(target=detect24, args=(conn1, conn4)),
        Process(target=whitebalance, args=(conn3, conn5)),
        Process(target=imwrite, args=(conn6, ))
    ]
   
    for process in processes:
        process.start()
    for process in processes:
        process.join()
    print("finished in {} seconds".format(time.time() - start_time))
    
